...
// Paystack removed - now using PayScribe Virtual Accounts
...
class DataPlan {
  ...
  factory DataPlan.fromJson(
    ...
    double parseCapacity(dynamic sizeString) {
      if (sizeString == null) return 0.0;
      final str = sizeString.toString().trim().toUpperCase();
      
      if (str == 'UNLIMITED') {
        return double.infinity;
      } else if (str.contains('GB')) {
        final value = str.replaceAll('GB', '').trim();
        return double.tryParse(value) ?? 0.0;
      } else if (str.contains('MB')) {
        final value = str.replaceAll('MB', '').trim();
        return (double.tryParse(value) ?? 0.0) / 1024;
      }
    ...
    return DataPlan(
      ...
      capacity: parseCapacity(json['data_size']),
      validity: 30,
      price: double.tryParse((json['sell_price'] ?? 0).toString()) ?? 0.0,
      efficiency: 100.0,
  ...
  factory HazPayLoan.fromJson(Map<String, dynamic> json) {
    return HazPayLoan(
      ...
      issuedAt: json['issued_at'] != null
          ...
          : null,
      repaidAt: json['repaid_at'] != null
          ? DateTime.parse(json['repaid_at'])
          : null,
...
class HazPayService {
    ...
    try {
      ...
      if (responseBody['success'] == true) {
        ...
        await supabase
            ...
            .update({'status': 'success', 'reference': data['reference']})
            .eq('id', transactionId);
  ...
  /// Deposit funds via PayScribe Virtual Account
  Future<Map<String, dynamic>?> depositToWallet(double amount) async {
    try {
      final userId = supabase.auth.currentUser?.id;
      if (userId == null) throw Exception('User not authenticated');
      ...
      debugPrint('💰 Creating PayScribe virtual account: ₦$amount');
      ...
      if (response.status != 200) {
        throw Exception('Failed to create virtual account: ${response.data}');
      }
      ...
      debugPrint('✅ Virtual account created: ${accountData['account_number']}');

      // Return account details for UI to display
      return {
  ...
  Future<Map<String, dynamic>> checkLoanEligibility() async {
    try {
      ...
      final totalSpent = (transactions as List).fold<double>(
        0,
  ...
  Future<Map<String, dynamic>> requestLoan({
    ...
    try {
      ...
      if (networkId != null) body['network'] = networkId.toString();
  ...
  Future<BillPayment> payElectricityBill({
    ...
    try {
      ...
      debugPrint(
        '⚡ Paying electricity bill: $discoCode, meter=$meterNumber, amount=₦$amount',
      );
      ...
      // Call PayScribe billPayment Edge Function
      ...
      final response = await supabase.functions.invoke(
        ...
        body: {
          ...
          'amount': amount,
          'phone': '08000000000', // Required by PayScribe
          'ref': billId,
        },
      );
  ...
  Future<BillPayment> payCableBill({
    ...
    required String planId, // e.g., 'GOHAN' for GOTV
    required double amount,
    ...
    try {
      ...
      debugPrint(
        '📺 Paying cable bill: $cableProvider, plan=$planId, smartcard=$smartcardNumber, amount=₦$amount',
      );
      ...
      // Call PayScribe billPayment Edge Function (uses /multichoice/vend)
      final response = await supabase.functions.invoke(
        'billPayment',
        body: {
          'bill_type': 'cable',
          'provider': cableProvider,
          'account': smartcardNumber, // Smartcard/IUC number
          'plan_id': planId, // PayScribe plan ID (e.g., 'GOHAN')
  ...
  Future<List<BillPayment>> getBillPaymentHistory({
    ...
    try {
      ...
      return (response as List).map((b) => BillPayment.fromJson(b)).toList();
    } catch (e) {
  ...
  /// Watch wallet balance in real-time
    ...
    try {
      ...
      return supabase
          ...
          .map((records) {
            ...
            return (records[0]['balance'] as num).toDouble();
          });
    } catch (e) {
      debugPrint('❌ Error watching wallet balance: $e');
      return Stream.value(0.0);
    }
  }
  ...
  Future<List<HazPayLoan>> getLoanHistory({
    ...
    try {
      ...
      return (response as List)
          .map((loan) => HazPayLoan.fromJson(loan))
          .toList();
    } catch (e) {
  ...
  Future<void> recordAdWatched() async {
    try {
      ...
      // Get or create today's ad watch record
      ...
      // Award points (1 point per ad)
      await supabase.rpc(
        'increment_user_points',
        params: {'p_user_id': userId, 'p_points': 1},
      );

      debugPrint('✅ Ad watched recorded + 1 point awarded');
    } catch (e) {
      debugPrint('❌ Error recording ad watch: $e');
    }
  }

  /// Redeem points for data
  Future<bool> redeemPointsForData({
    required int points,
    required int networkId,
    required String planId,
  }) async {
    try {
      ...
      if (userPoints < points) {
        throw Exception(
          'Insufficient points. You have $userPoints, need $points',
        );
      }
      ...
      // Purchase data (points-to-data conversion is typically: 5 points = 1 data unit)
      final dataUnits = points ~/ 5;
      debugPrint('✅ Redeemed $points points for $dataUnits data units');
  ...
  Future<int> getTodayAdCount() async {
    try {
      ...
      debugPrint('❌ Error getting today ad count: $e');
      return 0;
    }
  }

  /// Get recent ad watches
  Future<List<Map<String, dynamic>>> getRecentAdWatches({
    int limit = 30,
  }) async {
    try {
      final userId = supabase.auth.currentUser?.id;
      ...
      debugPrint('❌ Error getting recent ad watches: $e');
      return [];
    }
  }
  ...
  Future<void> _deductFromWallet(String userId, double amount) async {
    try {
      ...
      if (wallet.balance < amount) {
        throw Exception(
          'Insufficient wallet balance (have ₦${wallet.balance}, need ₦$amount)',
        );
      }
  ...
  String _generateId() {
    ...
    final uuid = values.map((v) => v.toRadixString(16).padLeft(2, '0')).join();
