...
interface StatusReplyNotificationPayload {
  fcm_token: string;
  status_id: string;
  replier_name: string;
  content: string;
...
serve(async (req) => {
  try {
    ...
    const payload: StatusReplyNotificationPayload = await req.json();
    ...
    const title = `${replier_name} replied to your status`;
    const body = reply_type === "emoji" ? `Sent emoji: ${content}` : content.substring(0, 100);
